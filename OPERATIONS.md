...
- **시스템 텔레메트리(사후분석용)**: `/workspace/telemetry.csv` — 30초마다 GPU(util/mem/temp/power)·시스템(mem/load)·디스크·train_running 기록. 별도 tmux `telem`(`runbook/08_telemetry.sh`). 학습 중 GPU 사용률 보면 데이터 병목 여부 진단 가능.
- ⚠️ full.log는 overwrite — 체인 **재시작 전엔 백업**(`cp full.log full.log.bak.$(date)`). 학습 상세·telemetry.csv는 append/timestamp라 보존.
